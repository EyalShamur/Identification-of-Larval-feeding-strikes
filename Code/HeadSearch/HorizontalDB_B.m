...
input_vid_dir = 'H:\thesis - eating fishes\DATABASES\Database-B\Videos-Compressed\';
input_clip_dir = 'H:\thesis - eating fishes\DATABASES\Database-D\DB_216';
output_Hor_dir = 'H:\thesis - eating fishes\DATABASES\Database-D\DB_216_compressed_rot';
if ~my_create_dir(output_Hor_dir); return; end;
output_Flip_dir = 'H:\thesis - eating fishes\DATABASES\Database-D\DB_216_compressed_rot_flip';
%output_Flip_dir = 'H:\thesis - eating fishes\DATABASES\Database-B\dummy';
if ~my_create_dir(output_Flip_dir); return; end;
%output_no_rot_dir = 'H:\thesis - eating fishes\DATABASES\Database-B\DB_no_rotation_frame=81x81_clip_len=30';
...
clip_idx
    ...
    %%%if clip_idx>=5
        input_filename = strcat('compressed_05_',input_filename,'.avi');
        input_vid = strcat(input_vid_dir,input_filename);
    %%%else 
        %input_vid = strcat(input_vid_dir,input_filename);
    %%%end
    %input_vid = strrep(input_vid, 'compressed_05_', '');
...
%}    
   ...
   % save fish head as video
    ...
    %implay(head_file);
    attributes_file = sprintf('%s/fish_head-%.4d.mat',input_clip_dir,clip_idx);
    frame_number = f_idx;
