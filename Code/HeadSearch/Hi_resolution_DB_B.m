...
input_vid_dir_compressed = 'H:\thesis - eating fishes\DATABASES\Database-B\Videos-Compressed\';
input_vid_dir_hi_res = 'H:\thesis - eating fishes\DATABASES\Database-B\Videos-orig\sparus feeding dextran\';
input_clip_dir = 'H:\thesis - eating fishes\DATABASES\Database-D\DB_216_compressed';
output_Hor_dir = 'H:\thesis - eating fishes\DATABASES\Database-D\DB_216_HI_RES_Rot_frame';
if ~my_create_dir(output_Hor_dir); return; end;
output_Flip_dir = 'H:\thesis - eating fishes\DATABASES\Database-D\DB_216_HI_RES_Rot_and_Flip_frame';
if ~my_create_dir(output_Flip_dir); return; end;
output_no_rot_dir = 'H:\thesis - eating fishes\DATABASES\Database-D\DB_216_HI_RES_no_rotation_frame';
if ~my_create_dir(output_no_rot_dir); return; end;
...
half_box_h=120;  % for saving head videos
half_box_w=120;  % for saving head videos    
    
    
for clip_idx=215:216
    clear mov;
    clear mov_flipped;
clip_idx
    ...
    if 0%clip_idx>=204
        input_vid_hi_res     = strcat(input_vid_dir_hi_res,'compressed_05_',input_filename,'.avi');
        input_vid_compressed = strcat(input_vid_dir_compressed,'compressed_05_',input_filename,'.avi');
    else 
        input_vid_hi_res = strcat(input_vid_dir_hi_res,input_filename);
        input_vid_compressed = strcat(input_vid_dir_compressed,input_filename);
    end
    input_vid_hi_res = strrep(input_vid_hi_res, 'compressed_05_', '');
    
    
    %input_vid = strcat(input_vid_dir,input_filename);
    input_filename = input_vid_hi_res;
    readerobj = VideoReader(input_vid_hi_res);
    readerobj_compressed = VideoReader(input_vid_compressed);
    n_1 = readerobj.NumberOfFrames;
    n_2 = readerobj_compressed.NumberOfFrames;
    compression_shift = uint32((frame_number*(n_1-n_2))/n_1);
compression_shift
    clear readerobj_compressed;
    
    f_idx = frame_number+compression_shift; 
    mouth_x = mouth_x*2;
    mouth_y = mouth_y*2;
