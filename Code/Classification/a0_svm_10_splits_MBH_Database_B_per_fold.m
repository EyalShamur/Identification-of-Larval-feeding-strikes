...
function [res] = a0_svm_10_splits_MBH_Database_B_per_fold(descriptor_type,descriptor_folder,model_type,database,use_hist_pca,folds_type,i_tries)
%close all
%clear all
...
for fold_idx=1:n_folds
    ...
    if use_hist_pca
        mbh_histogram_dir = sprintf('H:\\thesis - eating fishes\\DATABASES\\Database-%s\\%s\\%s\\PCA_HIST_SQRT\\test_num_%d',database,model_type,descriptor_folder,i_tries);
        ...
        mbh_histogram_dir = sprintf('H:\\thesis - eating fishes\\DATABASES\\Database-%s\\%s\\%s\\HISTOGRAM_1024\\test_num_%d',database,model_type,descriptor_folder,i_tries);
...
rmpath('C:\Program Files\MATLAB\R2013a\toolbox\stats\stats');
    model = svmtrain(training_label_vector, scaled_mat_train, ' -s 1 -t 2' );
