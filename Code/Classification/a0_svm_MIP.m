...
function [res] = a0_svm_MIP(run_id,descriptor_type,descriptor_folder,model_type,database,use_hist_pca,folds_type,i_tries,svm_kernel_type,run_id_output)
...
for fold_idx=1:n_folds
    ...
    if use_hist_pca
        mip_histogram_dir = sprintf('H:\\thesis - eating fishes\\DATABASES\\Database-%s\\%s\\%s\\RUN_ID_%d\\PCA_HIST_SQRT_ALL\\test_num_%d',database,model_type,descriptor_folder,run_id,i_tries);
        output_dir = sprintf('H:\\thesis - eating fishes\\DATABASES\\Database-%s\\%s\\%s\\RUN_ID_%d\\SVM_PCA\\test_num_%d',database,model_type,descriptor_folder,run_id_output,i_tries);
    else
        mip_histogram_dir = sprintf('H:\\thesis - eating fishes\\DATABASES\\Database-%s\\%s\\%s\\RUN_ID_%d\\HISTOGRAM\\test_num_%d',database,model_type,descriptor_folder,run_id,i_tries);
    ...
    mip_histogram_dir_for_training_set = sprintf('%s\\FOLD_%.2d_TRAINING_SET',mip_histogram_dir,fold_idx);
    mip_histogram_dir_for_test_set = sprintf('%s\\FOLD_%.2d_TEST_SET',mip_histogram_dir,fold_idx);
    ...
    for i=1:n_clips_for_training % training clips
        ...
        %msg = sprintf('***************  MIP: TRAINING : featur vector of fold %d indx %d clip %s  ***************\n',...
        %                                  fold_idx,i,f1); 
        %fprintf(msg); 
        local_i=local_i+1;
        ...
        hist_file = sprintf('%s/hist_7-%s.mat',mip_histogram_dir_for_training_set,f1);
...
training_label_vector(find(training_label_vector==0))=-1;
    fprintf(sprintf('\n\nSplit %d\n',fold_idx));
    fprintf('-------------\n');
    fprintf('\n\nrunning svmtrain...\n');
rmpath('C:\Program Files\MATLAB\R2013a\toolbox\stats\stats');
switches = strcat(' -s 1 ', svm_kernel_type);
    model = svmtrain(training_label_vector, scaled_mat_train, switches );
    ...
    for i=1:n_clips_for_test % training clips
        ...
        %msg = sprintf('***************  MIP: TEST : featur vector of fold %d indx %d clip %s  ***************\n',...
        %                                  fold_idx,i,f1);   
        %fprintf(msg); 
        local_i=local_i+1;
        ...
        hist_file = sprintf('%s/hist_7-%s.mat',mip_histogram_dir_for_test_set,f1);
        load (hist_file,'hist')  % hist
        nonnn = isnan(hist);
        idx_none=find(nonnn==1);
        if ~isempty(idx_none)
...
%}
    ...
    testing_label_vector(find(testing_label_vector==0))=-1;
...
% AUC
    ...
    f0 = find(testing_label_vector_all==0);
    testing_label_vector_all(f0)=-1;
