...
function [res last_fold_acc] = a0_svm(run_id, descriptor_type,descriptor_folder,model_type,database,use_hist_pca,folds_type,i_tries,svm_kernel_type,run_id_output)
%close all
%clear all
...
for fold_idx=1:n_folds
    ...
    if use_hist_pca
        mbh_histogram_dir = sprintf('H:\\thesis - eating fishes\\DATABASES\\Database-%s\\%s\\%s\\RUN_ID_%d\\PCA_HIST_SQRT\\test_num_%d',database,model_type,descriptor_folder,run_id,i_tries);
        output_dir = sprintf('H:\\thesis - eating fishes\\DATABASES\\Database-%s\\%s\\%s\\RUN_ID_%d\\SVM_PCA\\test_num_%d',database,model_type,descriptor_folder,run_id,i_tries);
    else
        mbh_histogram_dir = sprintf('H:\\thesis - eating fishes\\DATABASES\\Database-%s\\%s\\%s\\RUN_ID_%d\\HISTOGRAM\\test_num_%d',database,model_type,descriptor_folder,run_id,i_tries);
        output_dir = sprintf('H:\\thesis - eating fishes\\DATABASES\\Database-%s\\%s\\%s\\RUN_ID_%d\\SVM\\test_num_%d',database,model_type,descriptor_folder,run_id_output,i_tries);
    end
   ...
   %{         
    if A
        if histogram_1024
            mbh_histogram_dir = 'H:\thesis - eating fishes\DATABASES\Database-A\Models_Rot_and_Flip\MBH\HISTOGRAM_1024';
            mbh_histogram_dir_for_training_set = sprintf('%s\\FOLD_%.2d_TRAINING_SET',mbh_histogram_dir,fold_idx);
            mbh_histogram_dir_for_test_set = sprintf('%s\\FOLD_%.2d_TEST_SET',mbh_histogram_dir,fold_idx);
            
            output_dir = 'H:\thesis - eating fishes\DATABASES\Database-A\Models_Rot_and_Flip\MBH\MBH_SVM';
            if ~my_create_dir(output_dir); return; end;
        end
        if histogram_pca
            mbh_histogram_dir = 'H:\thesis - eating fishes\DATABASES\Database-A\Models_Rot_and_Flip\MBH\PCA_HIST_SQRT';
            mbh_histogram_dir_for_training_set = sprintf('%s\\FOLD_%.2d_TRAINING_SET',mbh_histogram_dir,fold_idx);
            mbh_histogram_dir_for_test_set = sprintf('%s\\FOLD_%.2d_TEST_SET',mbh_histogram_dir,fold_idx);
            output_dir = 'H:\thesis - eating fishes\DATABASES\Database-A\Models_Rot_and_Flip\MBH\MBH_SVM_PCA';
            if ~my_create_dir(output_dir); return; end; 
        end
    end
    ...
    if B
        if histogram_1024
            mbh_histogram_dir = 'H:\thesis - eating fishes\DATABASES\Database-B\Models_Rot_and_Flip\MBH\HISTOGRAM_1024';
            mbh_histogram_dir_for_training_set = sprintf('%s\\FOLD_%.2d_TRAINING_SET',mbh_histogram_dir,fold_idx);
            mbh_histogram_dir_for_test_set = sprintf('%s\\FOLD_%.2d_TEST_SET',mbh_histogram_dir,fold_idx);

            output_dir = 'H:\thesis - eating fishes\DATABASES\Database-B\Models_Rot_and_Flip\MBH\SVM';
            if ~my_create_dir(output_dir); return; end; 
        end
        if histogram_pca
            mbh_histogram_dir = 'H:\thesis - eating fishes\DATABASES\Database-B\Models_Rot_and_Flip\MBH\PCA_HIST_SQRT';
            mbh_histogram_dir_for_training_set = sprintf('%s\\FOLD_%.2d_TRAINING_SET',mbh_histogram_dir,fold_idx);
            mbh_histogram_dir_for_test_set = sprintf('%s\\FOLD_%.2d_TEST_SET',mbh_histogram_dir,fold_idx);
            
            output_dir = 'H:\thesis - eating fishes\DATABASES\Database-B\Models_Rot_and_Flip\MBH\SVM_PCA';
            if ~my_create_dir(output_dir); return; end; 
        end    
    end
%}
    ...
    fprintf(msg);
        ...
        hist_file = sprintf('%s/hist_7-%s.mat',mbh_histogram_dir_for_training_set,f1);
        ...
        hist = manipulate_hist(hist,descriptor_type);
...
rmpath('C:\Program Files\MATLAB\R2013a\toolbox\stats\stats');
    %switches = strcat(' -s 1 ', svm_kernel_type);
    model = svmtrain(training_label_vector, scaled_mat_train, svm_kernel_type );
...
addpath('C:\Program Files\MATLAB\R2013a\toolbox\stats\stats'); % perfcurve.m
    [X,Y,T,AUC] = perfcurve(testing_label_vector,decision_values(:,1),double(0));
...
%}
plot_auc_MATLAB_version = 0;
if plot_auc_MATLAB_version
    addpath('C:\Program Files\MATLAB\R2013a\toolbox\stats\stats'); % perfcurve.m
...
save(output_confusion_matrix_all,'positive_all','negative_all',...
   ...
   res = [ AUC accuracy_avg Standard_Error false_negative_ratio false_positive_ratio];
   
   testing_decision_values_all = decision_values_all(:,1);
    AUC_params_file = sprintf('%s/AUC_params.mat',output_dir);
    save(AUC_params_file,'testing_label_vector_all', 'testing_decision_values_all');

last_fold_acc = accuracy_all(n_folds);
end
