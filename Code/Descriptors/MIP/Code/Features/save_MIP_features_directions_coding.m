...
if(~isempty(directions))   
    ...
    for d = 1:length(directions)           
        ...
        [P,F] = get_MIP_points(mov_location,filename,MIP_params);
        F = sparse(double(F)); % All Frames MIP Features: size = 512*num of features.  (patches histograms=512) 
        P = single(P);  % All Frames Points: size = 3*num of features. (3 for : x,y,frame_idx)
