function [ hist, STIP_Bow_fn] = get_STIP_BoW(STIP_desc_dir,query_param)


% Output
    output_dir_norm = sprintf('%s\\NORM',STIP_desc_dir);
    output_dir_classes = sprintf('%s\\KMEANS',STIP_desc_dir);
    output_dir_Bow = sprintf('%s\\BoW',STIP_desc_dir);
...
%Input
    ...
    desc_folders=query_param.STIP_params.desc_folders;% = 'MBH_patch_size=32';
    model_type=query_param.model_type;% = 'Models_Rot_and_Flip';
    run_id=query_param.STIP_params.run_id;
    i_tries=query_param.i_tries;
    fold_idx=query_param.STIP_params.fold_idx;
...
% Norm
    ...
    fn = sprintf('%.4d',1);
    stip_file = strcat(STIP_desc_dir,'\STIP_7-',fn,'.mat');
    load (stip_file,'hoghof');  %'subspace_all_frames' first file
    hoghof = hoghof-min_all;
    desc_norm = uint8((hoghof/(max_all-min_all))*255);
    ...
    STIP_Bow_fn = sprintf('%s//BoW.mat',output_dir_Bow);
    save (STIP_Bow_fn,'hist')  % classes_2
    fclose('all');
